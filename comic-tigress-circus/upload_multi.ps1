param([Parameter(Mandatory=$true)][string]$FilePathsCsv)
$apiKey = $env:KIE_API_KEY
$results = @()
foreach ($FilePath in ($FilePathsCsv -split '\|')) {
    $boundary = [System.Guid]::NewGuid().ToString()
    $fileName = [System.IO.Path]::GetFileName($FilePath)
    $fileBytes = [System.IO.File]::ReadAllBytes($FilePath)
    $LF = "`r`n"
    $pre = "--$boundary$LF" + "Content-Disposition: form-data; name=`"uploadPath`"$LF$LF" + "tigra-comic$LF" + "--$boundary$LF" + "Content-Disposition: form-data; name=`"file`"; filename=`"$fileName`"$LF" + "Content-Type: image/png$LF$LF"
    $post = "$LF--$boundary--$LF"
    ...
    $results += [PSCustomObject]@{ name = $fileName; url = $url }
}
