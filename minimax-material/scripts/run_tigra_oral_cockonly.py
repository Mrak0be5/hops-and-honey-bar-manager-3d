...
OUT = Path(r"C:\Users\hebp\AppData\Roaming\StabilityMatrix\Images\Text2Img\Tigra_2026\oral_cockonly2")
PREFIX_DIR = "Tigra_2026/oral_cockonly2"
...
COCK = (
    "extreme close-up of tigress face, penis and testicles in muzzle, "
    "only penis and balls visible"
)

OUTFIT = "black sports bra, black crop top, black choker, bottomless"
...
NEG = (
    ...
    "(male body:1.9), (male torso:1.9), (man standing:1.8), (male legs:1.7), "
    "(male head:1.8), male hands, male arms, muscular man, "
    "(male tiger:2.0), (anthro male:2.0), (furry male:2.0), male tail, two tigers, "
    "(human lips:2.0), (plump lips:1.9), (pink lips:1.9), (fleshy lips:1.9), lipstick, lip gloss"
)

SHOTS = [
    ("kneel", f"in a gym, kneeling, fellatio, deepthroat, {OUTFIT}", 1512101),
    ("sit", f"in a gym, sitting on gym bench, leaning forward, fellatio, deepthroat, profile, {OUTFIT}", 1512102),
    ("close_profile", f"in a gym, close-up profile, kneeling, fellatio, deepthroat, muzzle at base of penis, side view, {OUTFIT}", 1512103),
    ("look_up", f"in a gym, kneeling, looking up, fellatio, deepthroat, profile view of face, {OUTFIT}", 1512104),
    ("sit_side", f"in a gym, sitting on gym bench, fellatio, deepthroat, side view, {OUTFIT}", 1512105),
]
...
def main():
    ...
    for i, (key, scene, seed) in enumerate(SHOTS, start=1):
        prefix = f"{PREFIX_DIR}/c2{i:02d}_{key}"
