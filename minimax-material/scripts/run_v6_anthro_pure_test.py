...
OUT = Path(r"C:\Users\hebp\AppData\Roaming\StabilityMatrix\Images\Text2Img\Tigra_2026\v6_anthro_pure_test")
COMFY_OUT = Path(r"C:\Users\hebp\AppData\Roaming\StabilityMatrix\Packages\ComfyUI\output")

# STRICT CANONICAL STACK (NO PIMPOCHKA LORA, NO CHEERLEADER)
WTP = 0.18
ANTHRO_M, ANTHRO_C = 0.85, 0.95  # Strong Anthro_Tiger_Mk_2
...
# Original TIGRA_BEST_2026 Character Prompt
CHAR = (
...
# HARD BLOCK ON HUMAN FACE & CHEERLEADER
BASE_NEG = (
...
def graph(act, pose_key, seed, prefix):
    ...
    if act == "oral":
        m, c = add_lora(nodes, "8", "facefuck_pony.safetensors", 0.15, 0.25, m, c)
...
def main():
    ...
    for act, pose, tag in tests:
        ...
        prefix = f"Tigra_2026/v6_anthro_pure_test/{act}_{tag}_{pose}"
        queue(graph(act, pose, seed, prefix))
        print(f"Queued {act} {tag} {pose} (seed={seed})", flush=True)
    
    print("Waiting for 5 V6 pure anthro test images...", flush=True)
    wait_queue()
    time.sleep(2)
    
    sub = COMFY_OUT / "Tigra_2026" / "v6_anthro_pure_test"
    if sub.exists():
    ...
    print("Done 5 V6 images.", flush=True)
