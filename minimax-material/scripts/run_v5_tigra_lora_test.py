...
OUT = Path(r"C:\Users\hebp\AppData\Roaming\StabilityMatrix\Images\Text2Img\Tigra_2026\v5_tigra_lora_test")
COMFY_OUT = Path(r"C:\Users\hebp\AppData\Roaming\StabilityMatrix\Packages\ComfyUI\output")

# Calibrated V5 Strengths
WTP = 0.18
TIGRA_LORA = 0.70    # Dedicated Tigra Pimpochka LoRA!
ANTHRO_M, ANTHRO_C = 0.60, 0.70  # Anthro Tiger support
...
# Canonical Tigra Trigger & Features
CHAR = (
    "score_9, score_8_up, score_9, score_8_up, score_7_up, score_6_up, illustration of wnw2, "
    "Tigra, orange cat, brown fur, "
    "((extreme detailed tiger woman face)), tiger paws, fur red tigger furry female nude, "
...
MALE = (
    ...
    "(human skin male:1.3), (smooth human skin male:1.3), muscular human male torso, male hands holding her hips, "
...
def graph(act, pose_key, seed, prefix):
    ...
    # 2. Dedicated Tigra LoRA!
    m, c = add_lora(nodes, "25", "Tigra_Pony_Pimpochka.safetensors", TIGRA_LORA, TIGRA_LORA, m, c)
    # 3. Anthro Tiger support
    ...
    if act == "oral":
        m, c = add_lora(nodes, "8", "facefuck_pony.safetensors", 0.18, 0.25, m, c)
        m, c = add_lora(nodes, "20", "Female_Tongue_Mouth_and_Teeth_-_PONY-v2.safetensors", 0.30, 0.30, m, c)
...
def main():
    ...
    # 5 Test Images for V5 with dedicated Tigra LoRA
    tests = [
        ("anal", "bench_press", "img1"),
        ("anal", "doggystyle", "img2"),
        ("anal", "squatting", "img3"),
        ("oral", "kneeling", "img4"),
        ("oral", "sitting_bench", "img5"),
    ]
    
    for act, pose, tag in tests:
        seed = rng.randint(1, 2**63 - 1)
        prefix = f"Tigra_2026/v5_tigra_lora_test/{act}_{tag}_{pose}"
        queue(graph(act, pose, seed, prefix))
        print(f"Queued {act} {tag} {pose} (seed={seed})", flush=True)
    
    print("Waiting for 5 V5 test images...", flush=True)
    wait_queue()
    time.sleep(2)
    
    sub = COMFY_OUT / "Tigra_2026" / "v5_tigra_lora_test"
    if sub.exists():
    ...
    print("Done 5 V5 approval images.", flush=True)
