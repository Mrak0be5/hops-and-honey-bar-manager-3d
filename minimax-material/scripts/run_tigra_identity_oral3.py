...
OUT = Path(r"C:\Users\hebp\AppData\Roaming\StabilityMatrix\Images\Text2Img\Tigra_2026\identity_oral3")
PREFIX_DIR = "Tigra_2026/identity_oral3"
...
NEG = (
    ...
    "shallow, tip only, half inserted, closed mouth, just posing, licking tip only, "
    ...
    "(futa:1.6), (catgirl:1.5), (kemonomimi:1.5), "
...
ORAL_BAN = (
    "fellatio", "blowjob", "deepthroat", "oral sex", "penis in mouth",
    "irrumatio", "facefuck", "cock in mouth", "sucking cock",
)

POSES = [
    ("kneeling_profile", "kneeling on gym floor, looking up, hands on his thighs, (profile view of face:1.45), (side view:1.35)", 937801),
    ("sitting_profile", "sitting on gym bench, leaning forward, (profile view of face:1.45), (side view:1.35)", 937802),
    ("close_profile", "close-up profile, kneeling, muzzle at base of penis, (side view of face:1.45)", 937803),
]
...
def graph(pose_desc, seed, prefix):
    ...
    if hits:
        raise RuntimeError(f"negative blocks oral: {hits}")
    if " oral" in f" {low}" or low.startswith("oral"):
        raise RuntimeError("negative contains oral")
    ...
    pos = (
        ...
        f"{MALE}, single image, one shot, no collage, no panels, 1girl, 1boy, "
        f"{work}, human male standing, only oral, {OUTFIT}\n"
        f"BREAK\n{CHAR}"
    )
    nodes.update({
...
def main():
    ...
    for i, (key, desc, seed) in enumerate(POSES, start=1):
        prefix = f"{PREFIX_DIR}/o{i:02d}_{key}"
        nodes, pos = graph(desc, seed, prefix)
        pid = queue(nodes)
        jobs.append({"id": i, "key": key, "seed": seed, "prefix": prefix, "prompt_id": pid, "prompt": pos, "negative": NEG})
