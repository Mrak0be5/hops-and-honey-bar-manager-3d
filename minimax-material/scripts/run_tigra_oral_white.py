...
OUT = Path(r"C:\Users\hebp\AppData\Roaming\StabilityMatrix\Images\Text2Img\Tigra_2026\oral_white")
PREFIX_DIR = "Tigra_2026/oral_white"
...
MALE = "white human male, fair-skinned man, Caucasian man"
...
NEG = (
    ...
    "(dark skin male:1.8), (black man:1.8), (brown skin male:1.6), ebony, dark-skinned man, "
    "(human lips:2.0), (plump lips:1.9), (pink lips:1.9), (fleshy lips:1.9), lipstick, lip gloss"
)

SHOTS = [
    ("kneel", f"in a gym, kneeling, fellatio, deepthroat, {MALE} standing, {OUTFIT}", 1602101),
    ("sit", f"in a gym, sitting on gym bench, leaning forward, fellatio, deepthroat, profile, {MALE} standing, {OUTFIT}", 1602102),
    ("close_profile", f"in a gym, close-up profile, kneeling, fellatio, deepthroat, muzzle at base of penis, side view, {MALE} standing, {OUTFIT}", 1602103),
    ("look_up", f"in a gym, kneeling, looking up, fellatio, deepthroat, profile view of face, {MALE} standing, {OUTFIT}", 1602104),
    ("sit_side", f"in a gym, sitting on gym bench, fellatio, deepthroat, side view, {MALE} standing, {OUTFIT}", 1602105),
]
...
def main():
    ...
    for i, (key, scene, seed) in enumerate(SHOTS, start=1):
        prefix = f"{PREFIX_DIR}/ow{i:02d}_{key}"
