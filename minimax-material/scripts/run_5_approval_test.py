...
OUT = Path(r"C:\Users\hebp\AppData\Roaming\StabilityMatrix\Images\Text2Img\Tigra_2026\v4_approval_test")
COMFY_OUT = Path(r"C:\Users\hebp\AppData\Roaming\StabilityMatrix\Packages\ComfyUI\output")

# Calibrated V4 Strengths
WTP = 0.18
...
def main():
    ...
    for act, pose, tag in tests:
        ...
        prefix = f"Tigra_2026/v4_approval_test/{act}_{tag}_{pose}"
        queue(graph(act, pose, seed, prefix))
        print(f"Queued {act} {tag} {pose} (seed={seed})", flush=True)
    
    print("Waiting for 5 approval test images...", flush=True)
    wait_queue()
    time.sleep(2)
    
    sub = COMFY_OUT / "Tigra_2026" / "v4_approval_test"
    if sub.exists():
    ...
    print("Done 5 approval images.", flush=True)
