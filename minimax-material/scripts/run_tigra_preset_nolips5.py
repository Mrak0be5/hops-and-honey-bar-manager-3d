...
OUT = Path(r"C:\Users\hebp\AppData\Roaming\StabilityMatrix\Images\Text2Img\Tigra_2026\preset_nolips")
PREFIX_DIR = "Tigra_2026/preset_nolips"
...
def graph(scene, seed, prefix):
    for banned in ("VAEEncodeForInpaint", "VAEEncode", "LoadImage", "SetLatentNoiseMask", "ImageToMask"):
        pass
    low = NEG.lower()
    ...
    while pending and time.time() - t0 < timeout:
        ...
        for pid in list(pending):
            ...
            if pid in hist:
                ...
                if st == "error" or hist[pid].get("status", {}).get("completed") is False and st == "error":
                    raise RuntimeError(f"job failed {pid}")
...
def main():
    ...
    for i, (key, scene, seed) in enumerate(SHOTS, start=1):
        prefix = f"{PREFIX_DIR}/pn{i:02d}_{key}"
