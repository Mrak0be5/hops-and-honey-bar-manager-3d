...
PREFIX_DIR = "Tigra_2026/dt_compare"
...
SHOTS = [
    ("oral_kneel", "in a gym, fitness center, kneeling, fellatio, deepthroat, human male standing, black sports bra, black crop top, black choker, bottomless", 982101),
    ("oral_sit", "in a gym, sitting on gym bench, leaning forward, fellatio, deepthroat, profile, human male standing, black sports bra, black crop top, black choker, bottomless", 982102),
    ("close_profile", "in a gym, close-up profile, kneeling, fellatio, deepthroat, muzzle at base of penis, side view, human male standing, black sports bra, black crop top, black choker, bottomless", 982103),
    ("kneel_look_up", "in a gym, kneeling on gym floor, looking up, fellatio, deepthroat, profile view of face, side view, human male standing, black sports bra, black crop top, black choker, bottomless", 982104),
    ("sit_side", "in a gym, sitting on gym bench, fellatio, deepthroat, side view, human male standing, black sports bra, black crop top, black choker, bottomless", 982105),
]
...
PIPES = [
    ...
    {
        "key": "anthro_oral",
        "lora": "ANTHRO_THROAT_FUCK.safetensors",
        "weight": 0.40,
        "extra": "Throatfuck",
        "url": "https://civitai.red/models/377361",
    },
    {
        "key": "murky",
        "lora": "MS_PDXL_Swallowing_Lite.safetensors",
        "weight": 0.50,
        "extra": "swallowingpose, balls deep",
        "url": "https://civitai.red/models/1602661",
    },
    {
        "key": "sword",
        "lora": "sword_swallowing_position_XL.safetensors",
        "weight": 0.40,
        "extra": "throatfuck",
        "url": "https://civitai.red/models/325951",
    },
]
...
def graph(pipe, scene, seed, prefix):
    nodes = {
    ...
    m, c = add_lora(nodes, "6", pipe["lora"], pipe["weight"], pipe["weight"], m, c)
    m, c = add_lora(nodes, "7", "m0d3rn_gym-p.safetensors", GYM, GYM, m, c)
    pos = f"{scene}, {pipe['extra']}, {MUZZLE},\n\n{CHAR}"
...
def wait_ids(ids, timeout=2400):
    ...
    while pending and time.time() - t0 < timeout:
        ...
        for pid in list(pending):
            ...
            if st == "error":
                print("FAILED", pid, (item.get("status") or {}).get("messages"), flush=True)
                done.append(pid)
                continue
        ...
        for pid in done:
            pending.discard(pid)
        if pending:
            print(f"waiting {len(pending)} jobs elapsed={int(time.time()-t0)}s", flush=True)
            time.sleep(10)
    if pending:
...
def main():
    ...
    ids = []
    for pipe in PIPES:
        (OUT / pipe["key"]).mkdir(parents=True, exist_ok=True)
        for i, (key, scene, seed) in enumerate(SHOTS, start=1):
            prefix = f"{PREFIX_DIR}/{pipe['key']}/{i:02d}_{key}"
            nodes, pos = graph(pipe, scene, seed, prefix)
            pid = queue(nodes)
            ids.append(pid)
            rec = {
                "pipe": pipe["key"],
            ...
            jobs.append(rec)
            print(f"queued {pipe['key']} {i} {key} {pid}", flush=True)
