...
$outDir = "C:\Users\hebp\OneDrive\Desktop\manager\hops-and-honey-bar-manager-3d\civitai-test-results\lora_deepthroat"
...
$models = @(
    @{
        ...
        ecosystem = "sdxl"
        lora = "urn:air:sdxl:lora:civitai:1116228@1254364" # deepthroat (girl lying)
        loraStrength = 0.85
    },
    @{
        id = "tovamix_v16"
        ...
        ecosystem = "sdxl"
        lora = "urn:air:sdxl:lora:civitai:1439937@2146834" # Deepthroat - Concept (booru v2)
        loraStrength = 0.8
    },
    @{
        id = "nova_furry_xl"
        ...
        ecosystem = "sdxl"
        lora = "urn:air:sdxl:lora:civitai:1281564@1445880" # Views of The Deepthroated
        loraStrength = 0.85
    }
)
...
$variations = @(
    @{
        varId = "v1"
        name = "Lying on back, reverse facefuck deepthroat"
        prompt = "masterpiece, best quality, ultra-detailed, 1girl, anthro tiger, orange fur, bold black tiger stripes, white muzzle chest stomach, short silver-white bob hair with bangs, bright green eyes, athletic six-pack abs, long ringed tiger tail, 1boy, lying on back, lying on floor, reverse facefuck, deepthroat, irrumatio, penis in mouth, cock in mouth, throatfuck, gagging, saliva, saliva trail, erect penis, green eyes locked on glowing TV screen in background, NOT looking at partner, NOT looking at camera, distracted by show, rating_explicit, studio lighting"
    },
    @{
        varId = "v2"
        name = "Upside-down head hanging, deepthroating viewed from above"
        prompt = "masterpiece, best quality, ultra-detailed, 1girl, anthro tiger, orange fur, bold black tiger stripes, white muzzle, short silver-white bob hair, bright green eyes, six-pack abs, long ringed tail, lying on back, head hanging off couch, inverted face, deepthroating viewed from above, deepthroat, irrumatio, penis inserted deep in mouth, throatfuck, gagging, saliva, erect penis, green eyes locked on glowing TV screen, NOT looking at him, NOT looking at camera, blue screen glow on face, rating_explicit, dynamic angle"
    }
)
...
foreach ($m in $models) {
    foreach ($v in $variations) {
        $jobLabel = "$($m.id)_$($v.varId)_lora"
        Write-Host "========================================"
        Write-Host "Generating with LoRA: $($m.title) - $($v.name) ($jobLabel)"
...
$resultsJson = Join-Path $outDir "results_lora_deepthroat.json"
